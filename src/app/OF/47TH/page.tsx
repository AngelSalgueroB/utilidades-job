...
const styles = StyleSheet.create({
  ...
  masterContainer: { flexDirection: 'row', gap: 10, marginBottom: 5, height: 115 }, 
  ...
  moWrapper: { marginTop: 'auto', flexDirection: 'row', borderWidth: 1, borderColor: '#cbd5e1', borderRadius: 4, overflow: 'hidden', backgroundColor: '#fff', height: 32 },
  ...
  td: { fontSize: 6.5, textAlign: 'center', paddingHorizontal: 2, color: '#334155', borderRightWidth: 1, borderRightColor: '#e2e8f0' },
});
  ...
  return (
    <Document>
      {Object.entries(groupedData).map(([soKey, rows], index) => {
        ...
        // Datos de Encabezado
        const clientName = manualClient || getValue(first, ['contractor', 'Customer', 'Cliente']) || 'CLIENTE';
            ...
            <View style={styles.masterContainer}>
                ...
                <View style={styles.leftColumn}>
                    ...
                    <View style={styles.infoRow}>
                        ...
                        <View style={{...styles.infoCard, flex: 1.5}}><Text style={styles.infoLabel}>Item Code / Callout</Text><Text style={styles.infoValue}>{itemCode}</Text></View>
                    </View>

                    {/* FILA 2 DE DATOS */}
                    <View style={styles.infoRow}>
                        <View style={styles.infoCard}><Text style={styles.infoLabel}>Prod ID</Text><Text style={styles.infoValue}>{prodId}</Text></View>
                        <View style={styles.infoCard}><Text style={styles.infoLabel}>Program</Text><Text style={styles.infoValue}>{program}</Text></View>
              ...
              {rows.map((row, idx) => (
                <View key={idx} style={{ ...styles.tableRow, backgroundColor: idx % 2 === 0 ? 'white' : '#f8fafc' }}>
                  {config.columns.map((col, i) => {
                    if (col.isCheck) {
                        return (
                            <View key={i} style={{ ...styles.td, flex: col.width, borderRightWidth: 1, borderRightColor: '#e2e8f0' }}><Text> </Text></View>
                        );
                    }
                    return (
...
export default function F47Page() {
  const [selectedProductId, setSelectedProductId] = useState<string>('47th_rfid');
  ...
  const processFiles = async () => {
    ...
    try {
        ...
        if (allData.length > 0) {
            ...
            const detectedClient = getValue(first, ['contractor', 'Customer', 'Cliente']);
  ...
  return (
    <main className="container mx-auto px-4 py-8 max-w-6xl">
      <div className="flex items-center justify-between mb-8">
        <Button variant="ghost" asChild className="text-orange-700 hover:bg-orange-50"><a href="/OF"><ArrowLeft className="mr-2 h-4 w-4" /> Volver al Menú</a></Button>
        <h1 className="text-3xl font-bold text-orange-700 flex items-center gap-3"><Package className="h-8 w-8" /> Generador 47TH</h1>
        <div className="w-80">
            <Label className="text-xs text-slate-500 mb-1 block">Producto:</Label>
            <Select value={selectedProductId} onValueChange={setSelectedProductId}>
                <SelectTrigger className="w-full border-orange-200 bg-white shadow-sm h-10"><SelectValue /></SelectTrigger>
                <SelectContent>{Object.values(F47_CONFIGS).map(p => (<SelectItem key={p.id} value={p.id}>{p.name}</SelectItem>))}</SelectContent>
            </Select>
        </div>
      </div>
      <div className="grid gap-6">
        <Card className="border-l-4 border-l-orange-500 shadow-md">
            <CardHeader className="pb-3"><CardTitle className="text-lg flex gap-2 items-center"><UploadCloud className="h-5 w-5 text-orange-600"/> Cargar Excel</CardTitle></CardHeader>
            <CardContent>
                <div className="flex items-center gap-4 mb-4">
                    <Button asChild variant="secondary" size="lg" className="bg-orange-50 hover:bg-orange-100 text-orange-700 border border-orange-200"><label className="cursor-pointer"><UploadCloud className="mr-2 h-5 w-5"/> Seleccionar Archivos<Input type="file" multiple className="hidden" accept=".xlsx,.xls,.csv" onChange={handleFileUpload} /></label></Button>
            ...
            <Card className="border-l-4 border-l-green-500 bg-green-50 shadow-md">
                ...
                <CardContent>
                    <div className="space-y-1 max-w-md">
                        ...
                        <Input id="clientInput" value={manualClient} onChange={(e) => setManualClient(e.target.value)} className="border-slate-300 bg-white"/>
                    </div>
                </CardContent>
