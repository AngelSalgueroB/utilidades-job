...
import { RIPLEY_CONFIGS, ProductConfig } from './config';
...
const styles = StyleSheet.create({
  ...
  tableRow: { flexDirection: 'row', borderBottomWidth: 1, borderBottomColor: '#e2e8f0', minHeight: 12, alignItems: 'center' },
  td: { fontSize: 5.5, textAlign: 'center', paddingHorizontal: 2, color: '#334155', borderRightWidth: 1, borderRightColor: '#e2e8f0' },
});

// ==================== COMPONENTE PDF ====================
const RipleyPDFDocument = ({ 
  ...
  return (
    <Document>
      {Object.entries(groupedData).map(([soKey, rows], index) => {
        ...
        const clientName = manualClient || getValue(first, ['contractor', 'Customer', 'Cliente']) || 'RIPLEY';
            ...
            <View style={styles.masterContainer}>
                <View style={styles.leftColumn}>
                    ...
                    <View style={styles.infoRow}>
                        ...
                        <View style={{...styles.infoCard, flex: 2}}><Text style={styles.infoLabel}>Label Name</Text><Text style={styles.infoValue}>{labelName}</Text></View>
                    </View>
            ...
            <View style={styles.tableContainer}>
              ...
              {rows.map((row, idx) => (
                <View key={idx} style={{ ...styles.tableRow, backgroundColor: idx % 2 === 0 ? 'white' : '#f8fafc' }}>
                  {activeColumns.map((col, i) => {
                    if (col.isCheck) {
                        return (<View key={i} style={{ ...styles.td, flex: col.width, borderRightWidth: 1, borderRightColor: '#e2e8f0' }}><Text> </Text></View>);
                    }
                    
                    return (
                        // @ts-ignore
                        <Text key={i} style={{ ...styles.td, flex: col.width, textAlign: col.align || 'center', fontWeight: col.isBold ? 'bold' : 'normal', borderRightWidth: 1, borderRightColor: '#e2e8f0' }} maxLines={1}>
...
export default function RipleyPage() {
  const [selectedProductId, setSelectedProductId] = useState<string>('ripley_rfid');
  ...
  const currentConfig = RIPLEY_CONFIGS[selectedProductId];
  ...
  const processFiles = async () => {
    ...
    try {
        for (const file of files) {
            ...
            allData = allData.concat(jsonWithQR);
        }
  ...
  const handleDownload = async () => {
    ...
    try {
        const blob = await pdf(<RipleyPDFDocument data={consolidatedData} manualClient={manualClient} imgBase64={imgBase64} logoBase64={logoBase64} config={currentConfig} />).toBlob();
  ...
  return (
    <main className="container mx-auto px-4 py-8 max-w-6xl">
      <div className="flex items-center justify-between mb-8">
        <Button variant="ghost" asChild className="text-violet-700 hover:bg-violet-50"><a href="/OF"><ArrowLeft className="mr-2 h-4 w-4" /> Volver al Menú</a></Button>
        <h1 className="text-3xl font-bold text-violet-700 flex items-center gap-3"><Package className="h-8 w-8" /> Generador Ripley</h1>
        <div className="w-80">
            <Select value={selectedProductId} onValueChange={setSelectedProductId}>
                <SelectTrigger className="w-full border-violet-200"><SelectValue /></SelectTrigger>
                <SelectContent>{Object.values(RIPLEY_CONFIGS).map(p => (<SelectItem key={p.id} value={p.id}>{p.name}</SelectItem>))}</SelectContent>
            </Select>
        </div>
      </div>
      <div className="grid gap-6">
        <Card className="border-l-4 border-l-violet-500 shadow-md">
            <CardHeader className="pb-3"><CardTitle className="text-lg flex gap-2"><UploadCloud className="h-5 w-5 text-violet-600"/> Cargar Excel</CardTitle></CardHeader>
            <CardContent>
        ...
        {showPreview && consolidatedData.length > 0 && (
            <Card className="border-l-4 border-l-green-500 bg-green-50 shadow-md">
                ...
                <CardContent><div className="space-y-1 max-w-md"><Label>CLIENTE (Editable)</Label><Input value={manualClient} onChange={(e) => setManualClient(e.target.value)} className="border-slate-300 bg-white"/></div></CardContent>
