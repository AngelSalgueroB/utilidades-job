      ...
      {Object.entries(groupedBySO).map(([soKey, rows], index) => {
        ...
        Object.entries(rowsByMO).forEach(([mo, groupRows]) => {
          ...
          groupRows.forEach((row) => {
            const size = String(
              getValue(row, ["Size", "size", "us_size"]) || "",
            );
            ...
            <View style={styles.infoGrid}>
              ...
              <View style={{ ...styles.infoCard, flex: 1.8 }}>
                ...
                >
                  ...
                  <Text style={{ fontSize: 9, color: "#64748b" }}>|</Text>
                  <Text style={{ ...styles.infoValue, ...styles.rmText }}>
                    {rmNo}
                  </Text>
                </View>
              </View>
            ...
            <View style={styles.middleSection}>
              <View style={styles.moContainer}>
                <View style={styles.moHeader}>
                  ...
                  <Text style={{ ...styles.moHeaderText, flex: 1.5 }}>
                    Size
                  </Text>
                  <Text
                    style={{
...
export default function TBPage() {
  ...
  const handleFolderUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
    if (event.target.files && event.target.files.length > 0) {
      ...
      const validFiles = files.filter(
        (f) => f.name.match(/\.(xlsx|xls)$/i) && !f.name.startsWith("~$"),
      );
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      ...
      link.href = url;
      link.download = `${currentConfig.id}_Report_${Date.now()}.pdf`;
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
    } catch (error) {
      console.error("Error:", error);
      setError("Error crítico generando PDF.");
    } finally {
    ...
    <main className="container mx-auto px-4 py-8 max-w-6xl">
      <div className="flex items-center justify-between mb-8">
        ...
        <h1 className="text-3xl font-bold text-indigo-700 flex items-center gap-3">
          <Package className="h-8 w-8" /> Generador TB RFID
        </h1>
      </div>
      ...
      <div className="grid gap-6">
        <Card className="border-l-4 border-l-blue-500 shadow-lg">
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              <Settings className="text-blue-500" /> Configuración Activa
            </CardTitle>
            <CardDescription>Producto: {currentConfig.name}</CardDescription>
          </CardHeader>
          <CardContent>
            <Button asChild variant="secondary" size="lg">
              <label className="cursor-pointer">
                <FolderSearch className="mr-2 h-4 w-4" /> Choose folder{" "}
                <Input
                  type="file"
                  className="hidden"
              ...
              <div className="flex items-center gap-2">
                {imgStatus === "ok" && (
                  <span className="text-green-600 font-bold flex items-center">
                    <CheckCircle2 className="h-4 w-4 mr-1" /> Preview imagen
                    ready.
                  </span>
                )}
                {imgStatus !== "ok" && (
        ...
        <Card
          className={`border-l-4 shadow-lg ${filesToProcess.length > 0 ? "border-l-green-500" : ""}`}
        >
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
        ...
        {showPreview && consolidatedData.length > 0 && isClient && (
          <Card className="border-l-4 border-l-indigo-500 bg-indigo-50">
            <CardHeader>
              ...
              <CardDescription>
                {consolidatedData.length} registros listos.
              </CardDescription>
            </CardHeader>
            <CardFooter>
              <Button
